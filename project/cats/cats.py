...
def about(subject):
    ...
    def select(p):
        for x in subject:
            if x in split( lower( remove_punctuation(p) ) ):
                return True
        return False
    return select
    # END PROBLEM 2
    ...
    if len(typed_words)==0:
        ...
        for index in range(min_len):
            if typed_words[index]==source_words[index]:
                match_cnt += 1
        return match_cnt * 100.0 / len(typed_words)
    # END PROBLEM 3
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        ...
        for index in range(len(words)):
            times[i] += [timestamps_per_player[i][index+1] - timestamps_per_player[i][index]]
    ...
    for word_index in word_indices:
        ...
        for player_index in player_indices:
            if time(match, player_index, word_index)<min_time:
                ...
                index = player_index
        words_list[index] += [get_word(match, word_index)]
    return words_list
    # END PROBLEM 10
